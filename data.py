...
class Tracking3DVideoDataset(Dataset):
    ...
    def __getitem__old(self, idx):
        ...
        if frames_video.empty:
            raise IndexError(f"No label entries for trajectory {trajectory}")
        images = []
        labels = []
        transformation = None
        if self.split == "train" and self.augment:
            ...
            if random.random() > 0.5 and self.rotation:
                transformation = "rotate"
                angle = random.uniform(-15, 15)
        video_file = os.path.join(self.slices_dir, f"tr{str(trajectory)}" + ".avi")
        ...
        frames = frames[:, :, :, [0, 2]].transpose(1, 3).transpose(2, 3) # Convert to format [time, channels, height, width]
        ...
        for _, row in frames_video.iterrows():
            ...
            label = np.array([row[label] for label in self.labels])
            try:
                image_tensor = frames[n_frame]
            except IndexError:
                break

            if self.crop_margins: image_tensor, label = self.crop_item(image_tensor, label)
            x, y, r = label // self.label_quantization
            r = min(r, MAX_VAL_R_CAM // self.label_quantization)  # Clip the radius to a maximum value
            ...
            if pd.isna(x) or pd.isna(y) or pd.isna(r):
                label = torch.tensor([-1, -1, -1], dtype=torch.float32)
                print(f"Warning: Missing label for {n_frame}")
            else:
                if transformation == "hflip":
                    image_tensor = TF.hflip(image_tensor)
                    x = width - x
                ...
                # if x < 0 or y < 0 or x >= width or y >= height: # NOW I AM NOT CLIPPING TO NOT INTERFERE WITH THE TRAJECTORY
                #     print(f"Warning: Label out of bounds for {frame_name}, x={x}, y={y}")
                #     x = min(width, max(0, x))
                #     y = min(height, max(0, y))
                #     label = torch.tensor([x, y, r], dtype=torch.float32)
